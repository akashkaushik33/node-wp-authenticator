...
WP_Auth.prototype.getUserMeta = function( id, key, callback ) {
  ...
  var self = this;
  this.db.query( 'select meta_value from ' + this.table_prefix + 'usermeta where meta_key = \'' + sanitizeValue( key ) + '\' and user_id = ' + parseInt( id ), function(err, results ) {
...
WP_Auth.prototype.reverseUserMeta = function( key, value, callback ) {
  ...
  var self = this;
  this.db.query( 'select user_id from ' + this.table_prefix + 'usermeta where meta_key = \'' + sanitizeValue( key ) + '\' and meta_value = \'' + sanitizeValue( key ) + '\'' ,function( err, results ) {
    var data = results && results[0] || {}
    id = data.user_id;
    if ( !( id in self.meta_cache ) )
      self.meta_cache[id] = {};
...
WP_Auth.prototype.getContributors = function( callback ) {
  ...
  this.db.query( 'select user_id, user_login from ' + this.table_prefix + 'usermeta as meta left join ' + this.table_prefix + 'users as user on user.ID = meta.user_id where meta_key = \'wp_user_level\' and meta_value = \'1\'' , function(err, results ) {
    var data = results && results[0] || {}
    users.push( data );
    //end
    callback( users );
...
exports.create = function(config) {
    const {wpurl, logged_in_key, logged_in_salt, mysql_host, mysql_user, mysql_pass, mysql_port, mysql_db, wp_table_prefix} = config
return new WP_Auth( wpurl, logged_in_key, logged_in_salt,
    mysql_host, mysql_user, mysql_pass, mysql_port, mysql_db,
    wp_table_prefix );
};